...
GpuIndexInt8::GpuIndexInt8(GpuResources* resources,
                           int dims,
                           faiss::MetricType metric,
                           GpuIndexConfig config) :
        IndexInt8(dims, metric),
...
void GpuIndexInt8::copyFrom(const faiss::IndexInt8 *index) {
    d = index->d;
...
void GpuIndexInt8::copyTo(faiss::IndexInt8 *index) const {
    index->d = d;
...
void GpuIndexInt8::setMinPagingSize(size_t size) {
    minPagedSize_ = size;
}

size_t GpuIndexInt8::getMinPagingSize() const {
    return minPagedSize_;
}

void GpuIndexInt8::add(faiss::Index::idx_t n, const int8_t *x) {
    // Pass to add_with_ids
    add_with_ids(n, x, nullptr);
}

void GpuIndexInt8::add_with_ids(Index::idx_t n, const int8_t *x, const Index::idx_t *ids) {
...
void GpuIndexInt8::addPaged_(int n, const int8_t *x, const Index::idx_t *ids) {
    if (n > 0) {
        size_t totalSize = (size_t) n * this->d;

        if (totalSize > kAddPageSize || n > kAddVecSize) {
            // How many vectors fit into kAddPageSize?
            size_t maxNumVecsForPageSize =
                    kAddPageSize / (size_t) this->d;
...
void GpuIndexInt8::addPage_(int n, const int8_t *x, const Index::idx_t *ids) {
    ...
    auto vecs = toDevice<int8_t, 2>(resources_,
                                   device_,
                                   const_cast<int8_t *>(x),
                                   stream,
                                   {n, this->d});
...
void GpuIndexInt8::search(Index::idx_t n, const int8_t *x, Index::idx_t k, int *distances, Index::idx_t *labels) const {
    FAISS_THROW_IF_NOT_MSG(this->is_trained, "Index not trained");
    ...
    auto outDistances =
            toDevice<int, 2>(resources_, device_, distances, stream,
                             {(int) n, (int) k});
...
void GpuIndexInt8::searchNonPaged_(int n, const int8_t *x, int k, int *outDistancesData,
...
void GpuIndexInt8::searchFromCpuPaged_(int n, const int8_t *x, int k, int *outDistancesData,
                                       Index::idx_t *outIndicesData) const {
    ...
    int pageSizeInVecs =
            (int) ((pinnedAlloc.second / 2) / this->d);
    ...
    if (!pinnedAlloc.first || pageSizeInVecs < 1) {
        ...
        int batchSize = utils::nextHighestPowerOf2(
                (int) ((size_t) kNonPinnedPageSize / this->d));
    ...
    int8_t* bufPinnedA = (int8_t*) pinnedAlloc.first;
    int8_t* bufPinnedB = bufPinnedA + (size_t) pageSizeInVecs * this->d;
    int8_t* bufPinned[2] = {bufPinnedA, bufPinnedB};

    // Reserve space on the GPU for the destination of the pinned buffer
    // copy
    DeviceTensor<int8_t, 2, true> bufGpuA(
            resources_->getMemoryManagerCurrentDevice(),
            {(int) pageSizeInVecs, (int) this->d},
            defaultStream);
    DeviceTensor<int8_t, 2, true> bufGpuB(
            resources_->getMemoryManagerCurrentDevice(),
            {(int) pageSizeInVecs, (int) this->d},
            defaultStream);
    DeviceTensor<int8_t, 2, true>* bufGpus[2] = {&bufGpuA, &bufGpuB};
